...
import atexit
import logging
from type import CollectionName
...
API_KEY = _get_env("API_KEY", required=False)  # optional; don't print secrets
...
def create_mongo_client(
    ...
    while attempt < max_retries:
        try:
            logger.info("Attempting to connect to MongoDB (attempt %d/%d)...", attempt + 1, max_retries)
            client = MongoClient(
                uri,
            ...
            if API_KEY:
                logger.info("API_KEY found in environment (value not logged for security).")
            else:
    ...
    logger.critical("Could not connect to MongoDB after %d attempts. Exiting.", max_retries)
    if last_exc:
